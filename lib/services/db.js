...
const log = require('../logger')().child({type: 'db'})
...
module.exports = function createDb () {
  ...
  return Object.assign(db, {
    name: 'db', // Name of service
    getConnection () {
      return mongoose.connection
    },

    /**
     * Connects to mongodb and throws event when it is ready.
     * @param {object} [DB_URL=config['DB_URL]] The database url
     * @return {EventEmitter}
     */
     ...
     */
    disconnect () {
      mongoose.disconnect(_ => db.emit('disconnected'))
      return db
    }
  })
}
